...
Matrix<4, 4, T> lookAt(T eyeX, T eyeY, T eyeZ, T centerX, T centerY, T centerZ,
	...
	//set camera x-axis
	Vector < 3, T > upvec(upX, upY, upZ);
	Vector<3, T> xvec = cross(zvec, upvec);
	...
	//set camera y-axis
	Vector<3, T> yvec = cross(xvec, zvec);
	...
	m(3, 3) = static_cast<T>(1); //set R inverse Matrixrix
